...
prompt_mypure_human_time_to_var() {
...
# Stores (into prompt_mypure_cmd_exec_time) the execution
# time of the last command if set threshold was exceeded.
prompt_mypure_check_cmd_exec_time() {
  integer elapsed
  (( elapsed = EPOCHSECONDS - ${prompt_mypure_cmd_timestamp:-$EPOCHSECONDS} ))
  typeset -g prompt_mypure_cmd_exec_time=
  (( elapsed > ${PURE_CMD_MAX_EXEC_TIME:-20} )) && {
    prompt_mypure_human_time_to_var $elapsed "prompt_mypure_cmd_exec_time"
  }
}

prompt_mypure_set_title() {
  ...
  local hostname=
  if [[ -n $prompt_mypure_state[username] ]]; then
...
prompt_mypure_preexec() {
  if [[ -n $prompt_mypure_git_fetch_pattern ]]; then
    # Detect when Git is performing pull/fetch, including Git aliases.
    local -H MATCH MBEGIN MEND match mbegin mend
    if [[ $2 =~ (git|hub)\ (.*\ )?($prompt_mypure_git_fetch_pattern)(\ .*)?$ ]]; then
      # We must flush the async jobs to cancel our git fetch in order
      # to avoid conflicts with the user issued pull / fetch.
      async_flush_jobs 'prompt_mypure'
    fi
  fi

  typeset -g prompt_mypure_cmd_timestamp=$EPOCHSECONDS

  # Shows the current directory and executed command in the title while a process is active.
  prompt_mypure_set_title 'ignore-escape' "$PWD:t: $2"
...
prompt_mypure_set_colors() {
  local color_temp key value
  for key value in ${(kv)prompt_mypure_colors}; do
    ...
    case $? in
      1) # The current style is different from the one from zstyle.
        ...
        prompt_mypure_colors[$key]=$color_temp ;;
      2) # No style is defined.
        prompt_mypure_colors[$key]=$prompt_mypure_colors_default[$key] ;;
    esac
  done
}

prompt_mypure_set_aws() {
...
prompt_mypure_preprompt_render() {
  setopt localoptions noshwordsplit

  # Set color for Git branch/dirty status and change color if dirty checking has been delayed.
  local git_color=$prompt_mypure_colors[git:branch]
  local git_dirty_color=$prompt_mypure_colors[git:dirty]
  [[ -n ${prompt_mypure_git_last_dirty_check_timestamp+x} ]] && git_color=$prompt_mypure_colors[git:branch:cached]

  # Initialize the preprompt array.
  local -a preprompt_parts

  [[ $UID -eq 0 ]] && preprompt_parts+='%F{$prompt_mypure_colors[user:root]}'"⚡"

  [[ -n $prompt_mypure_state[username] ]] && preprompt_parts+='%F{$prompt_mypure_colors[host]}'"|%m|"
  # [[ ! -z $SSH_CONNECTION ]] && preprompt_parts+='%F{$prompt_mypure_colors[host]}'"@%m"

  prompt_mypure_set_aws
  if [[ ! -z $prompt_mypure_aws ]]; then
    if [[ $prompt_mypure_aws_prod -eq 1 ]];then
      preprompt_parts+="%F{$prompt_mypure_colors[aws:prod]}"'$prompt_mypure_aws'
    else
      preprompt_parts+="%F{$prompt_mypure_colors[aws]}"'$prompt_mypure_aws'
    fi
  fi
  ...
  if [[ -n $prompt_mypure_vcs_info[branch] ]]; then
    local branch="%F{$git_color}"'${prompt_mypure_vcs_info[branch]}'
    if [[ -n $prompt_mypure_vcs_info[action] ]]; then
      branch+="|%F{$prompt_mypure_colors[git:action]}"'$prompt_mypure_vcs_info[action]'"%F{$git_color}"
    fi
    preprompt_parts+=("$branch""%F{$git_dirty_color}"'${prompt_mypure_git_dirty}%f')
  fi
  # Git pull/push arrows.
  if [[ -n $prompt_mypure_git_arrows ]]; then
    preprompt_parts+=('%F{$prompt_mypure_colors[git:arrow]}${prompt_mypure_git_arrows}%f')
  fi

  # # Username and machine, if applicable.
  # [[ -n $prompt_mypure_state[username] ]] && preprompt_parts+=($prompt_mypure_state[username])
  # Execution time.
  [[ -n $prompt_mypure_cmd_exec_time ]] && preprompt_parts+=('%F{$prompt_mypure_colors[execution_time]}${prompt_mypure_cmd_exec_time}%f')

  jobno=$(jobs|wc -l)
  # [[ $jobno -ne 0 ]] && preprompt_parts+='%F{$prompt_mypure_colors[bgjobs]}'"$jobno☢"
  [[ $jobno -ne 0 ]] && preprompt_parts+='%F{$prompt_mypure_colors[bgjobs]}'"☢"
  # [[ $jobno -ne 0 ]] && preprompt_parts+='%F{$prompt_mypure_colors[bgjobs]}'"!"
  ...
  # if [[ $1 == precmd ]]; then
    ...
    # print
  # elif [[ $prompt_mypure_last_prompt != $expanded_prompt ]]; then
    # Redraw the prompt.
    prompt_mypure_reset_prompt
  # fi



  typeset -g prompt_mypure_last_prompt=$expanded_prompt
}

prompt_mypure_precmd() {
  # Check execution time and store it in a variable.
  prompt_mypure_check_cmd_exec_time
  unset prompt_mypure_cmd_timestamp

  # Shows the full path in the title.
  prompt_mypure_set_title 'expand-prompt' '%2.'

  # Modify the colors if some have changed..
  prompt_mypure_set_colors

  # Perform async Git dirty check and fetch.
  prompt_mypure_async_tasks
  ...
  prompt_mypure_reset_prompt_symbol

  # Print the preprompt.
  prompt_mypure_preprompt_render "precmd"
...
prompt_mypure_async_git_aliases() {
  setopt localoptions noshwordsplit
...
prompt_mypure_async_git_fetch() {
  ...
  prompt_mypure_async_git_arrows
}
...
prompt_mypure_async_tasks() {
  ...
  ((!${prompt_mypure_async_init:-0})) && {
    async_start_worker "prompt_mypure" -u -n
    async_register_callback "prompt_mypure" prompt_mypure_async_callback
    typeset -g prompt_mypure_async_init=1
    async_job "prompt_mypure" prompt_mypure_async_renice
  }

  # Update the current working directory of the async worker.
  async_worker_eval "prompt_mypure" builtin cd -q $PWD

  typeset -gA prompt_mypure_vcs_info

  local -H MATCH MBEGIN MEND
  if [[ $PWD != ${prompt_mypure_vcs_info[pwd]}* ]]; then
    # Stop any running async jobs.
    async_flush_jobs "prompt_mypure"

    # Reset Git preprompt variables, switching working tree.
    unset prompt_mypure_git_dirty
    unset prompt_mypure_git_last_dirty_check_timestamp
    unset prompt_mypure_git_arrows
    unset prompt_mypure_git_fetch_pattern
    prompt_mypure_vcs_info[branch]=
    prompt_mypure_vcs_info[top]=
  fi
  unset MATCH MBEGIN MEND

  async_job "prompt_mypure" prompt_mypure_async_vcs_info

  # Only perform tasks inside a Git working tree.
  [[ -n $prompt_mypure_vcs_info[top] ]] || return

  prompt_mypure_async_refresh
}

prompt_mypure_async_refresh() {
  setopt localoptions noshwordsplit

  if [[ -z $prompt_mypure_git_fetch_pattern ]]; then
    # We set the pattern here to avoid redoing the pattern check until the
    # working three has changed. Pull and fetch are always valid patterns.
    typeset -g prompt_mypure_git_fetch_pattern="pull|fetch"
    async_job "prompt_mypure" prompt_mypure_async_git_aliases
  fi

  async_job "prompt_mypure" prompt_mypure_async_git_arrows

  # Do not preform `git fetch` if it is disabled or in home folder.
  if (( ${PURE_GIT_PULL:-1} )) && [[ $prompt_mypure_vcs_info[top] != $HOME ]]; then
    # Tell the async worker to do a `git fetch`.
    async_job "prompt_mypure" prompt_mypure_async_git_fetch
  fi

  # If dirty checking is sufficiently fast,
  # tell the worker to check it again, or wait for timeout.
  integer time_since_last_dirty_check=$(( EPOCHSECONDS - ${prompt_mypure_git_last_dirty_check_timestamp:-0} ))
  if (( time_since_last_dirty_check > ${PURE_GIT_DELAY_DIRTY_CHECK:-1800} )); then
    unset prompt_mypure_git_last_dirty_check_timestamp
    # Check check if there is anything to pull.
    async_job "prompt_mypure" prompt_mypure_async_git_dirty ${PURE_GIT_UNTRACKED_DIRTY:-1}
  fi
}

prompt_mypure_check_git_arrows() {
  setopt localoptions noshwordsplit
...
prompt_mypure_async_callback() {
  ...
  case $job in
    \[async])
      ...
      if [[ $code -eq 2 ]]; then
        ...
        typeset -g prompt_mypure_async_init=0
      fi
      ;;
    prompt_mypure_async_vcs_info)
      local -A info
      ...
      if [[ $info[top] = $prompt_mypure_vcs_info[top] ]]; then
        # If the stored pwd is part of $PWD, $PWD is shorter and likelier
        # to be top-level, so we update pwd.
        if [[ $prompt_mypure_vcs_info[pwd] = ${PWD}* ]]; then
          prompt_mypure_vcs_info[pwd]=$PWD
        fi
      else
      ...
      [[ -n $info[top] ]] && [[ -z $prompt_mypure_vcs_info[top] ]] && prompt_mypure_async_refresh

      # Always update branch and top-level.
      prompt_mypure_vcs_info[branch]=$info[branch]
      prompt_mypure_vcs_info[top]=$info[top]
      prompt_mypure_vcs_info[action]=$info[action]
    ...
    prompt_mypure_async_git_aliases)
      if [[ -n $output ]]; then
        ...
        prompt_mypure_git_fetch_pattern+="|$output"
      fi
      ;;
    prompt_mypure_async_git_dirty)
      local prev_dirty=$prompt_mypure_git_dirty
      if (( code == 0 )); then
        unset prompt_mypure_git_dirty
      else
      ...
      # When `prompt_mypure_git_last_dirty_check_timestamp` is set, the Git info is displayed
      ...
      (( $exec_time > 5 )) && prompt_mypure_git_last_dirty_check_timestamp=$EPOCHSECONDS
      ;;
    prompt_mypure_async_git_fetch|prompt_mypure_async_git_arrows)
      # `prompt_mypure_async_git_fetch` executes `prompt_mypure_async_git_arrows`
      # after a successful fetch.
      case $code in
        0)
          local REPLY
          prompt_mypure_check_git_arrows ${(ps:\t:)output}
          if [[ $prompt_mypure_git_arrows != $REPLY ]]; then
            typeset -g prompt_mypure_git_arrows=$REPLY
            do_render=1
          fi
          ;;
        97)
          ...
          if [[ -n $prompt_mypure_git_arrows ]]; then
            typeset -g prompt_mypure_git_arrows=
            do_render=1
          fi
          ;;
        99|98)
          ...
          ;;
        *)
          # Non-zero exit status from `prompt_mypure_async_git_arrows`,
  ...
  if (( next_pending )); then
    (( do_render )) && typeset -g prompt_mypure_async_render_requested=1
    return
  fi

  [[ ${prompt_mypure_async_render_requested:-$do_render} = 1 ]] && prompt_mypure_preprompt_render
  unset prompt_mypure_async_render_requested
}
...
prompt_mypure_reset_prompt_symbol() {
  prompt_mypure_state[prompt]=${PURE_PROMPT_SYMBOL:-❯}
}

prompt_mypure_update_vim_prompt_widget() {
  setopt localoptions noshwordsplit
  prompt_mypure_state[prompt]=${${KEYMAP/vicmd/${PURE_PROMPT_VICMD_SYMBOL:-❮}}/(main|viins)/${PURE_PROMPT_SYMBOL:-❯}}

  prompt_mypure_reset_prompt
}

prompt_mypure_reset_vim_prompt_widget() {
  setopt localoptions noshwordsplit
  prompt_mypure_reset_prompt_symbol
...
prompt_mypure_state_setup() {
	setopt localoptions noshwordsplit

	# Check SSH_CONNECTION and the current state.
	local ssh_connection=${SSH_CONNECTION:-$prompt_mypure_SSH_CONNECTION}
	...
	if [[ -z $ssh_connection ]] && (( $+commands[who] )); then
		...
		if [[ $who_out =~ "\(?($reIPv4|$reIPv6|$reHostname)\)?\$" ]]; then
			...
			export prompt_mypure_SSH_CONNECTION=$ssh_connection
		fi
		unset MATCH MBEGIN MEND
	fi

	hostname='%F{$prompt_mypure_colors[host]}@%m%f'
	# Show `username@host` if logged in through SSH.
	[[ -n $ssh_connection ]] && username='%F{$prompt_mypure_colors[user]}%n%f'"$hostname"

	# # Show `username@host` if root, with username in default color.
	# [[ $UID -eq 0 ]] && username='%F{$prompt_mypure_colors[user:root]}%n%f'"$hostname"

  typeset -gA prompt_mypure_state
  prompt_mypure_state[version]="1.11.0"
  prompt_mypure_state+=(
    username "$username"
...
prompt_mypure_setup() {
  ...
  # Set the colors.
  typeset -gA prompt_mypure_colors_default prompt_mypure_colors
  prompt_mypure_colors_default=(
  ...
  prompt_mypure_colors=("${(@kv)prompt_mypure_colors_default}")

  add-zsh-hook precmd prompt_mypure_precmd
  add-zsh-hook preexec prompt_mypure_preexec

  prompt_mypure_state_setup

  zle -N prompt_mypure_reset_prompt
  zle -N prompt_mypure_update_vim_prompt_widget
  zle -N prompt_mypure_reset_vim_prompt_widget
  if (( $+functions[add-zle-hook-widget] )); then
    add-zle-hook-widget zle-line-finish prompt_mypure_reset_vim_prompt_widget
    add-zle-hook-widget zle-keymap-select prompt_mypure_update_vim_prompt_widget
  fi

  # If a virtualenv is activated, display it
  PROMPT='%(12V.%F{$prompt_mypure_colors[virtualenv]}%12v%f.)'

  # Add iterm2 mark 
  PROMPT+=%{$(iterm2_prompt_mark)%}

  # Prompt turns red if the previous command didn't exit with 0.
  local prompt_indicator='%(?.%F{$prompt_mypure_colors[prompt:success]}.%F{$prompt_mypure_colors[prompt:error]})${prompt_mypure_state[prompt]}%f '
  PROMPT+=$prompt_indicator

  # Indicate continuation prompt by … and use a darker color for it.
  PROMPT2='%F{$prompt_mypure_colors[prompt:continuation]}… %(1_.%_ .%_)%f'$prompt_indicator

  # Store prompt expansion symbols for in-place expansion via (%). For
  # some reason it does not work without storing them in a variable first.
  typeset -ga prompt_mypure_debug_depth
  prompt_mypure_debug_depth=('%e' '%N' '%x')
  ...
  local -A ps4_parts
  ps4_parts=(
    depth     '%F{yellow}${(l:${(%)prompt_mypure_debug_depth[1]}::+:)}%f'
    compare   '${${(%)prompt_mypure_debug_depth[2]}:#${(%)prompt_mypure_debug_depth[3]}}'
    main      '%F{blue}${${(%)prompt_mypure_debug_depth[3]}:t}%f%F{242}:%I%f %F{242}@%f%F{blue}%N%f%F{242}:%i%f'
...
prompt_mypure_system_report() {
  ...
  print - "- Pure state:"
  for k v in "${(@kv)prompt_mypure_state}"; do
    print - "    - $k: \`${(q)v}\`"
  done
  print - "- PROMPT: \`$(typeset -p PROMPT)\`"
  print - "- Colors: \`$(typeset -p prompt_mypure_colors)\`"
